...
r_FramebufferTexture::r_FramebufferTexture()
	...
	, wrap_mode_( WRAP_MODE_REPEAT )
	, filter_min_( FILTRATION_NEAREST_MIPMAP_LINEAR )
	, filter_mag_( FILTRATION_LINEAR )
	, size_x_(0), size_y_(0)
{
}
...
{
	return (
			format_ == FORMAT_DEPTH16  ||
			format_ == FORMAT_DEPTH32  ||
			format_ == FORMAT_DEPTH32F ||
			format_ == FORMAT_DEPTH24_STENCIL8 );
}
